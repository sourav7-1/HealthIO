...
| 14 | Medication safety | ✅ Core done ([docs/phases/14-medication-safety.md](docs/phases/14-medication-safety.md)): deterministic engine for duplicate medicines and ingredients, interactions, contraindications, allergy conflicts and prescription consistency, using only imported trusted datasets (none shipped); stored warnings with source, severity, timestamps and review status; doctor review and patient acknowledgement; safety check before issuing a prescription. Dose-limit and age/pregnancy checks remain |
| 15–25 | | Planned |
