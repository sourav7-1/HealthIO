...
| AI document reading | prescription photos sent to the vision model; AI reading, review trail | Filling in prescriptions from photos | Consent: platform purpose `ai_processing` (separate, optional, withdrawable; guardians consent for dependants) | Photo and reading kept with the record; provider zero-retention required | Phase 8 |
...
| Anthropic (Claude API) | Prescription photos (re-encoded, metadata stripped) and OCR text of the page; no name, ID or other records. Chat (Phase 13) | To confirm (cross-border transfer, DPDP s.16); zero-retention settings required before enabling in production | 8, 13 |
